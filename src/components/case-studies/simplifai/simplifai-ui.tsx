...
export function BrowserFrame({ url, badge, children }: { url: string; badge?: string; children: React.ReactNode }) {
    return (
        <div className="rounded-2xl overflow-hidden border border-zinc-200 shadow-2xl bg-white w-full">
            <div className="flex items-center gap-3 px-4 py-3 bg-zinc-100 border-b border-zinc-200">
                ...
                <div className="flex-1 bg-white border border-zinc-200 rounded-lg px-3 py-1 text-xs text-zinc-600 text-center truncate">{url}</div>
                {badge && <span className="text-[10px] font-bold text-violet-700 bg-violet-50 border border-violet-200 px-2 py-0.5 rounded-full flex-shrink-0">{badge}</span>}
            </div>
            {children}
        </div>
    );
...
function FlowDashboard() {
    const flows = [
        ...
        { name: "Invoice Processing Pipeline", desc: "Extracts line items, routes for approval, syncs to ERP", trigger: "Email", status: "Active", runs: "1 hr ago", color: "bg-blue-100 text-blue-800" },
        { name: "Customer Support Triage", desc: "Classifies tickets, assigns priority, notifies on-call", trigger: "Webhook", status: "Active", runs: "3 hrs ago", color: "bg-emerald-100 text-emerald-800" },
        { name: "Contract Renewal Alert", desc: "Monitors contract dates, triggers 30/14/7-day reminders", trigger: "Schedule", status: "Draft", runs: "Never", color: "bg-zinc-100 text-zinc-600" },
    ];
            ...
            <div className="w-[200px] border-r border-zinc-100 bg-zinc-50 flex flex-col flex-shrink-0">
                ...
                <nav className="flex-1 p-2 space-y-0.5">
                    {[
                        ...
                        <div key={item.label} className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm cursor-pointer ${item.active ? "bg-violet-600 text-white font-semibold" : "text-zinc-600 hover:bg-zinc-100"}`}>
                            <span>{item.label}</span>
                            {item.dot && <span className={`text-[10px] font-bold ${item.active ? "bg-white/20 text-white" : "bg-zinc-200 text-zinc-600"} px-1.5 py-0.5 rounded-full`}>{item.dot}</span>}
                        </div>
                    ))}
                </nav>
                {/* User */}
                <div className="flex items-center gap-2 px-4 py-3 border-t border-zinc-100">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[9px] text-zinc-600">Pro Plan</div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                    <div>
                        ...
                        <p className="text-xs text-zinc-600 mt-0.5">4 flows · 3 active</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2 border border-zinc-200 rounded-lg px-3 py-1.5 text-xs text-zinc-600">
                ...
                {/* Flow list */}
                <div className="flex-1 overflow-auto p-4 space-y-2" tabIndex={0}>
                    {flows.map((f) => (
                        <div key={f.name} className="flex items-center gap-4 px-4 py-3 rounded-xl border border-zinc-100 hover:border-zinc-200 hover:shadow-sm transition-all cursor-pointer bg-white group">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-zinc-600 truncate">{f.desc}</div>
                            </div>
                            <div className="flex items-center gap-3 flex-shrink-0">
                                <div className="text-[10px] font-medium text-zinc-600 border border-zinc-100 rounded px-1.5 py-0.5">{f.trigger}</div>
                                <div className="text-[10px] text-zinc-600">{f.runs}</div>
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${f.status === "Active" ? "bg-emerald-50 text-emerald-600" : "bg-zinc-100 text-zinc-600"}`}>{f.status}</span>
                                <div className="text-zinc-300 group-hover:text-zinc-600 text-lg leading-none">···</div>
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="mt-2 pt-4 border-t border-zinc-100">
                        <div className="text-xs font-bold text-zinc-600 uppercase tracking-widest mb-3">Quick Start Templates</div>
                        <div className="flex gap-2">
                            {["Lead Scoring", "Invoice Automation", "Support Triage", "Data Enrichment"].map((t) => (
                                <div key={t} className="px-3 py-2 rounded-lg border border-dashed border-zinc-200 text-xs text-zinc-600 hover:border-violet-300 hover:text-violet-700 cursor-pointer transition-colors">{t}</div>
                            ))}
                        </div>
                    </div>
                </div>
        ...
        <div className="flex flex-col bg-white" style={{ height: 400 }}>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-200 bg-white flex-shrink-0">
                <div className="flex items-center gap-2 text-sm font-semibold text-zinc-700">
                    <span className="text-zinc-600">My Flows</span>
                ...
                <div className="flex items-center gap-2">
                    <div className="text-xs text-zinc-600 border border-zinc-200 rounded px-2 py-1">100%</div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                >
                    ...
                    <div className="absolute bottom-3 left-3 flex items-center gap-1">
                        ...
                        <div className="bg-white border border-zinc-200 rounded-lg px-2 py-1.5 shadow-sm text-[10px] text-zinc-600">Fit</div>
                    </div>
                ...
                <div className="w-[160px] border-l border-zinc-200 bg-white flex flex-col flex-shrink-0">
                    <div className="px-3 py-2.5 border-b border-zinc-100">
                        <div className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mb-1">Selected Node</div>
                        <div className="text-xs font-bold text-zinc-800">Webhook Trigger</div>
                    </div>
                    <div className="p-3 space-y-2.5 flex-1">
                        <div>
                            <label className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 block mb-1">Method</label>
                            <div className="bg-violet-50 border border-violet-200 rounded-md px-2 py-1.5 text-xs font-bold text-violet-700">POST</div>
                        </div>
                        <div>
                            <label className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 block mb-1">Endpoint</label>
                        ...
                        <div>
                            <label className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 block mb-1">Auth</label>
                            <div className="bg-zinc-50 border border-zinc-200 rounded-md px-2 py-1.5 text-xs text-zinc-600">Bearer Token</div>
                        </div>
                        <div>
                            <label className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 block mb-1">Payload Schema</label>
...
function ExecutionMonitor() {
    ...
    const statusStyles: Record<string, { pill: string; dot: string }> = {
        Completed: { pill: "bg-emerald-50 text-emerald-800 border-emerald-200", dot: "bg-emerald-500" },
        Running: { pill: "bg-blue-50 text-blue-800 border-blue-200", dot: "bg-blue-500 animate-pulse" },
    ...
    return (
        <div className="flex flex-col bg-white" style={{ height: 400 }}>
            ...
            <div className="flex items-center gap-3 px-6 py-3 border-b border-zinc-100 bg-white flex-shrink-0">
                <div className="flex items-center gap-2 text-xs text-zinc-600 border border-zinc-200 rounded-lg px-3 py-1.5">
                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><circle cx="5" cy="5" r="3.5" stroke="#9ca3af" strokeWidth="1.2"/><path d="M8 8l2 2" stroke="#9ca3af" strokeWidth="1.2" strokeLinecap="round"/></svg>
                    Search executions...
                </div>
                <div className="flex gap-1.5">
                    {["All", "Completed", "Running", "Failed"].map((f, i) => (
                        <span key={f} className={`text-xs font-semibold px-2.5 py-1 rounded-full border cursor-pointer ${i === 0 ? "bg-zinc-900 text-white border-zinc-900" : "border-zinc-200 text-zinc-600 hover:border-zinc-300"}`}>{f}</span>
                    ))}
                </div>
                <div className="ml-auto text-xs text-zinc-600 flex items-center gap-1.5">
                    <span>Last 24h</span>
                    <span className="text-zinc-300">·</span>
            ...
            {/* Table header */}
            <div className="grid grid-cols-5 gap-2 px-6 py-2 text-[10px] font-bold uppercase tracking-widest text-zinc-600 border-b border-zinc-100 bg-zinc-50/50 flex-shrink-0">
            ...
            {/* Rows */}
            <div className="flex-1 overflow-auto divide-y divide-zinc-50" tabIndex={0}>
                {runs.map((r) => (
                    <div key={r.id}>
                        <div className="grid grid-cols-5 gap-2 px-6 py-3 items-center hover:bg-zinc-50/50 cursor-pointer">
                            <div className="col-span-2 flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <div className="text-[10px] text-zinc-600 font-mono">{r.id}</div>
                                </div>
                            </div>
                            <div>
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusStyles[r.status].pill}`}>{r.status}</span>
                            </div>
                            <div className="text-xs text-zinc-600 font-mono">{r.steps}</div>
                            <div className="text-xs text-zinc-600 font-mono">{r.dur}</div>
                        </div>
                        {r.expanded && (
                            <div className="mx-6 mb-3 rounded-xl bg-[#0d1117] p-3 border border-zinc-800">
                                <div className="text-[9px] font-bold uppercase tracking-widest text-zinc-600 mb-2">Execution Trace</div>
                                <div className="space-y-1 text-[10px] font-mono">
                                    <div className="text-blue-400">→ Webhook received: POST /webhook/lead {"{ email: \"j.smith@corp.com\" }"}</div>
                                    <div className="text-zinc-600">→ Clearbit enrich: matched ✓ · Company: Corp Inc · Size: 500-1000</div>
        ...
        <div>
            ...
            <div className="flex gap-1 mb-6 border-b border-zinc-200">
                {tabs.map((tab, i) => (
                    <button
                        key={i}
                        onClick={() => setActive(i)}
                        className={`relative px-4 py-2.5 text-sm font-semibold transition-colors rounded-t-lg ${active === i ? "text-violet-700" : "text-zinc-600 hover:text-zinc-600"}`}
                    >
                        {tab.shortLabel}
                        {active === i && (
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <p className="text-sm text-zinc-600 mt-4 leading-relaxed max-w-3xl">{tabs[active].caption}</p>
                </motion.div>
            </AnimatePresence>
        </div>
    );
