...
export function Testimonials() {
    return (
        <div className="w-full py-20 overflow-hidden relative">
            <div className="absolute left-0 top-0 bottom-0 w-20 bg-gradient-to-r from-zinc-50/80 to-transparent z-10 pointer-events-none" />
            <div className="absolute right-0 top-0 bottom-0 w-20 bg-gradient-to-l from-zinc-50/80 to-transparent z-10 pointer-events-none" />

            <div className="flex w-max animate-marquee hover:[animation-play-state:paused]">
                {[...testimonials, ...testimonials, ...testimonials, ...testimonials].map((testimonial, i) => (
                    <Card
                        key={i}
                        className="w-[350px] shrink-0 mx-4 p-6 bg-white/50 backdrop-blur-xl border-zinc-200"
                    >
                        <p className="text-zinc-700 text-sm italic mb-4">"{testimonial.quote}"</p>
